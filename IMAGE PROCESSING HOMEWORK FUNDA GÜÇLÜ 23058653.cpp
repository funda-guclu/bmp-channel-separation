...
void processAndSaveChannel(IMAGE* originalImage, char channelType, const char* outputFilename) {
    IMAGE* newImage = (IMAGE*)malloc(sizeof(IMAGE));
    if (!newImage) {
        printf("ERROR: Memory could not be allocated for the new picture.\n");
        return;
    }

    newImage->bmpfh = originalImage->bmpfh;
    newImage->bmpih = originalImage->bmpih;

    LONG w = newImage->bmpih.biw;
    LONG h = abs(newImage->bmpih.bih);
    DWORD row8_size = ((w * 8 + 31) / 32) * 4;
    DWORD data8_size = row8_size * h;

    newImage->bmpih.bibitcount = 8;
    newImage->bmpih.bisizeimage = data8_size;
    newImage->bmpih.biclused = 256;
    newImage->bmpih.biclimp = 256;
    newImage->bmpfh.bfOffbits = sizeof(BMPFH) + sizeof(BMPIH) + 256 * sizeof(PALET);
    newImage->bmpfh.bfsize = newImage->bmpfh.bfOffbits + data8_size;

    newImage->palet = createPalette(channelType);
    newImage->data = (BYTE*)malloc(data8_size);
    DWORD row24_size = ((w * 24 + 31) / 32) * 4;
    for (int y = 0; y < h; y++) {
        for (int x = 0; x < w; x++) {
            int idx24 = y * row24_size + x * 3;
            int idx8 = y * row8_size + x;
            if (channelType == 'r') newImage->data[idx8] = originalImage->data[idx24 + 2];
            else if (channelType == 'g') newImage->data[idx8] = originalImage->data[idx24 + 1];
            else if (channelType == 'b') newImage->data[idx8] = originalImage->data[idx24 + 0];
        }
    }
    
    ImageWrite(newImage, (char*)outputFilename);
...
int main()
{
    ...
    const char *inputFilename = "fruit.bmp"; 

    originalImage = ImageRead(originalImage, (char*)inputFilename);
    if (!originalImage) return 1;
    if (originalImage->bmpih.bibitcount != 24) {
    ...
    processAndSaveChannel(originalImage, 'r', "red8.bmp");
    processAndSaveChannel(originalImage, 'g', "green8.bmp");
    processAndSaveChannel(originalImage, 'b', "blue8.bmp");
